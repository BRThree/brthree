...
import {BreathParameterData, CubismBreath} from "@Cubism4/framework/effect/cubismbreath.ts";
 ...
 * @param filePath
 */
export const loadSetting = async (filePath: string): Promise<ICubismModelSetting> => {
  const response = await fetch(filePath);
  const arrayBuffer = await response.arrayBuffer();
  return new CubismModelSettingJson(
    arrayBuffer,
 ...
 * @param filePath
 */
export const loadModel = async (setting: ICubismModelSetting, filePath: string): Promise<CubismModel> => {
 ...
 * @param setting
 */
export const setupEyeBlink = (setting: ICubismModelSetting): CubismEyeBlink | null => {
 ...
 * @param setting
 */
export const setupLipSyncIds = (setting: ICubismModelSetting): csmVector<CubismIdHandle> => {
 ...
 * @param modelMatrix
 */
export const setupLayout = (setting: ICubismModelSetting, modelMatrix: CubismModelMatrix): void => {
