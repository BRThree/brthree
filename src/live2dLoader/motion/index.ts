...
import {ACubismMotion, FinishedMotionCallback} from "@Cubism4/framework/motion/acubismmotion.ts";
...
const preLoadMotionGroup = async (
  ...
  for (let i = 0; i < setting.getMotionCount(group); i++) {
    ...
    const tmpMotion: CubismMotion = CubismMotion.create(arrayBuffer, arrayBuffer.byteLength);
...
const startMotion = (
  ...
  return motionManager.startMotionPriority(
    motion,
    autoDelete,
    priority
  );
