import {
    ...
    LogLevel,
    CubismFramework,
} from '@Cubism4/framework/live2dcubismframework';
import {
    AssetsResult,
    EyeBlinkInitResult,
    Live2DInitOpt,
} from '../type/Live2DLoaderType';
import {
    loadCubismPhysics,
    loadCubismPose,
    loadModel,
...
const assetsInit = async (
    ...
    const expressions = (await loadCubismExpression(
        setting,
        filePath
    )) as csmMap<string, ACubismMotion>;
...
const eyeBlinkInit = (setting: ICubismModelSetting): EyeBlinkInitResult => {
    ...
    const eyeBlinkIdCount: number = setting.getEyeBlinkParameterCount();
...
const init = async (opt: Live2DInitOpt) => {
    const { jsonPath, canvas, size, switchExpression } = opt;
    // 初始化WebGL
    const live2dContainer: HTMLCanvasElement =
        canvas || document.createElement('canvas');
    const gl: WebGLRenderingContext = live2dContainer.getContext(
        'webgl'
    ...
    const { motionManager, motions } = await loadCubismMotion(
        model,
        setting,
        gl,
        BASE_URL,
    ...
    loop(gl, live2dContainer, frameBuffer, () => {
        setModelScale(
            model,
            modelMatrix,
            live2dContainer.width,
            live2dContainer.height
        );
