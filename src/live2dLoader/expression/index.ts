...
export const loadCubismExpression = async (
    ...
    for (let i = 0; i < count; i++) {
        ...
        const motion: ACubismMotion = CubismExpressionMotion.create(
            arrayBuffer,
            arrayBuffer.byteLength
        );

        if (expressions.getValue(expressionName) != null) {
            ACubismMotion.delete(expressions.getValue(expressionName));
        }
...
export const setExpression = (
    ...
    if (motion != null) {
        expressionManager.startMotionPriority(motion, false, 3);
    }
};
