...
// 只暴露方法，不暴露实例
export const get = <T = undefined, R = undefined>(url: string, payload: T): Promise<AxiosResponse<Result<R>>> => {
  return axiosInstance.get(url, {params: payload})
}
export const post = <T = undefined, R = undefined>(url: string, payload: T): Promise<AxiosResponse<Result<R>>> => {
  return axiosInstance.post(url, {data: payload})
}
export const put = <T = undefined, R = undefined>(url: string, payload: T): Promise<AxiosResponse<Result<R>>> => {
  return axiosInstance.put(url, {data: payload})
}
export const del = <T = undefined, R = undefined>(url: string, payload: T): Promise<AxiosResponse<Result<R>>> => {
