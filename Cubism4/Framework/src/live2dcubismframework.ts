...
export class CubismFramework {
  /**
   * 可使用Cubism Framework的API。
   *  在执行API之前一定要执行这个函数。
   *  一旦准备完毕，即使再次执行，内部处理也会被跳过。
   *
   * @param    option      Option类的实例
   *
   * @return   准备处理完成后返回true。
   */
  ...
  /**
   * 对Cubism Framework内的资源进行初始化，使其成为可以显示模型的状态。<br>
   *     再次Initialize()需要先执行Dispose()。
   *
   * @param memorySize 初期化時メモリ量 [byte(s)]
   *    请在显示复数模型时等模型没有被更新的时候使用。
  ...
  public static initialize(memorySize = 0): void {
    ...
    // --- HACK: 初始化时的存储量扩展(単位byte) ---
    // 请在模型未更新时使用，例如显示多个模型时。
    // 指定する際は必ず1024*1024*16 byte(16MB)以上の値を指定してください。
    // それ以外はすべて1024*1024*16 byteに丸めます。
